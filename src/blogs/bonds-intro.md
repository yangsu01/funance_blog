...
Even though the payments and the face value of a bond does not change, expected returns (yield) does. This is represented in the bond price changing, think about _present value of money_ for y'all econ nerds. But unlike stocks, the fluctuation can be more easily understood, as it is often the result of... Interest rate changes.

A general rule of thumb is: **Bond prices move in the opposite direction of interest rates**.
...
This is one way it can be show, in a similar layout to stock quotes. I've also seen it presented in a bond table (I found it kinda hard to find real-time bond prices when researching for this post...). Regardless of how it is presented, the core information stays the same:

- `Yield to Maturity (YtM)` - the total expected return of a bond if it is held to maturity. It is often just noted as `yield`, not to be confused from the `current yield` (The annualized rate of return on a bond). For the rest of the blog, when I mention _yield_, I will be referring to the _yield to maturity_
...
- `Coupon` - the annual or semi-annual amount of money that the issuer will pay to the holder up until maturity. Kinda like periodic interest payments (Canadian government bonds have semi-annual payouts). Coupon rate is predetermined and will not change. Often represented as a percentage of the PAR
...
OK, if you know what these terms mean, you're golden. But lets dig a bit deeper...
...
Lets **_visualize_**. Choosing the _Canada 1 Year Bond_ and _Canada 5 Year Bond_, we can calculate the cash flows of each bond assuming we bought it today. Note, as previously mentioned, we need to convert the clean price to the dirty price to account for any accrued interest.
...
| CAN 3-MO  | 4.660% |      4.618%      | 0.901288%  |
| CAN 3-MO  | 4.032% |      4.025%      | 0.173611%  |
| CAN 3-MO  | 3.815% |      3.815%      | 0.000000%  |
| CAN 3-MO  | 3.639% |      3.643%      | 0.109920%  |
| CAN 3-MO  | 3.584% |      3.584%      | 0.000000%  |
| CAN 3-MO  | 3.567% |      3.567%      | 0.000000%  |
| CAN 3-MO  | 3.458% |      3.480%      | 0.636206%  |

_Table 1. Difference of Calculated Yield and Quoted Yield_
...
During a recession, investors tend to seek safe long term investments by purchasing long maturity bonds over short ones. By supply and demand, this increases the price of long term bonds and causes its yield to dip below shorter term bonds. Heres a great video by [_The Plain Bagel_](https://www.youtube.com/watch?v=5L_zQGPNXOk&t=445s&ab_channel=ThePlainBagel) that explains this.
...
The relationship between interest rate and price can be though of as an arbitrary function. How can we approximate it? Well, **Taylor expansions** of course!
